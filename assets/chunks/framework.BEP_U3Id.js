...
const isInternalKey = (key) => key[0] === "_" || key === "$stable", normalizeSlotValue = (value) => isArray(value) ? value.map(normalizeVNode) : [normalizeVNode(value)], normalizeSlot = (key, rawSlot, ctx) => {
  ...
  if (instance.vnode.shapeFlag & 32) {
    ...
    type ? (extend(slots, children), def(slots, "_", type, !0)) : normalizeObjectSlots(children, slots);
  } else
    children && normalizeVNodeSlots(instance, children);
...
function cloneVNode(vnode, extraProps, mergeRef = !1, cloneTransition = !1) {
  ...
  return transition && cloneTransition && (cloned.transition = transition.clone(cloned)), cloned;
}
...
function resolveTransitionProps(rawProps) {
  ...
  return extend(baseProps, {
    ...
    onLeave(el, done) {
      ...
      addTransitionClass(el, leaveFromClass), addTransitionClass(el, leaveActiveClass), forceReflow(), nextFrame(() => {
...
const assignKey = Symbol("_assign"), vModelText = {
  ...
  beforeUpdate(el, { value, modifiers: { lazy, trim, number } }, vnode) {
    if (el[assignKey] = getModelAssigner(vnode), el.composing)
      return;
    const elValue = (number || el.type === "number") && !/^0\d/.test(el.value) ? looseToNumber(el.value) : el.value, newValue = value ?? "";
...
function getLocaleForPath(siteData2, relativePath) {
  return Object.keys((siteData2 == null ? void 0 : siteData2.locales) || {}).find((key) => key !== "root" && !isExternal(key) && isActive(relativePath, `/${key}/`, !0)) || "root";
}
...
function initData(route) {
  ...
  }) : ref(!1), hashRef = ref(inBrowser ? location.hash : "");
  return inBrowser && window.addEventListener("hashchange", () => {
...
function createRouter(loadPageModule, fallbackComponent) {
  ...
  async function go(href = inBrowser ? location.href : "/") {
    var _a, _b;
    href = normalizeHref(href), await ((_a = router.onBeforeRouteChange) == null ? void 0 : _a.call(router, href)) !== !1 && (inBrowser && href !== normalizeHref(location.href) && (history.replaceState({ scrollPosition: window.scrollY }, ""), history.pushState({}, "", href)), await loadPage(href), await ((_b = router.onAfterRouteChanged) == null ? void 0 : _b.call(router, href)));
  }
    ...
    try {
      ...
      if (latestPendingPath === pendingPath) {
        ...
        route.path = inBrowser ? pendingPath : withBase(pendingPath), route.component = markRaw(comp), route.data = markRaw(__pageData), inBrowser && nextTick(() => {
          ...
          if (!siteDataRef.value.cleanUrls && !actualPathname.endsWith("/") && (actualPathname += ".html"), actualPathname !== targetLoc.pathname && (targetLoc.pathname = actualPathname, href = actualPathname + targetLoc.search + targetLoc.hash, history.replaceState({}, "", href)), targetLoc.hash && !scrollPosition) {
            let target = null;
            try {
      ...
      if (latestPendingPath === pendingPath) {
        latestPendingPath = null, route.path = inBrowser ? pendingPath : withBase(pendingPath), route.component = fallbackComponent ? markRaw(fallbackComponent) : null;
        const relativePath = inBrowser ? pendingPath.replace(/(^|\/)$/, "$1index").replace(/(\.html)?$/, ".md").replace(/^\//, "") : "404.md";
        route.data = { ...notFoundPageData, relativePath };
      }
    }
  }
  return inBrowser && (history.state === null && history.replaceState({}, ""), window.addEventListener("click", (e) => {
    ...
    if (link2 && !link2.closest(".vp-raw") && (link2 instanceof SVGElement || !link2.download)) {
      ...
      !e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey && !target && origin === currentUrl.origin && treatAsHtml(pathname) && (e.preventDefault(), pathname === currentUrl.pathname && search === currentUrl.search ? (hash !== currentUrl.hash && (history.pushState({}, "", href), window.dispatchEvent(new HashChangeEvent("hashchange", {
        oldURL: currentUrl.href,
        newURL: href
      }))), hash ? scrollTo$1(link2, hash, link2.classList.contains("header-anchor")) : window.scrollTo(0, 0)) : go(href));
    }
  }, { capture: !0 }), window.addEventListener("popstate", async (e) => {
    var _a;
    e.state !== null && (await loadPage(normalizeHref(location.href), e.state && e.state.scrollPosition || 0), (_a = router.onAfterRouteChanged) == null || _a.call(router, location.href));
