...
function createRouter(loadPageModule, fallbackComponent) {
  ...
  return inBrowser && (window.addEventListener("click", (e) => {
    ...
    if (link2 && !link2.closest(".vp-raw") && (link2 instanceof SVGElement || !link2.download)) {
      const { target } = link2, { href, origin, pathname, hash, search } = new URL(link2.href instanceof SVGAnimatedString ? link2.href.animVal : link2.href, link2.baseURI), currentUrl = new URL(window.location.href);
      !e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey && !target && origin === currentUrl.origin && treatAsHtml(pathname) && (e.preventDefault(), pathname === currentUrl.pathname && search === currentUrl.search ? (hash !== currentUrl.hash && (history.pushState(null, "", hash), window.dispatchEvent(new Event("hashchange"))), hash ? scrollTo$1(link2, hash, link2.classList.contains("header-anchor")) : (updateHistory(href, !1), window.scrollTo(0, 0))) : go(href));
    }
...
function updateHistory(href, emitHashChange = !0) {
  if (inBrowser && normalizeHref(href) !== normalizeHref(location.href)) {
    const currentHash = location.hash;
    history.replaceState({ scrollPosition: window.scrollY }, document.title), history.pushState(null, "", href), emitHashChange && new URL(href, fakeHost).hash !== currentHash && window.dispatchEvent(new Event("hashchange"));
  }
}
