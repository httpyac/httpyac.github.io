...
<<< @../../examples/script/postRequestScript.http{5 HTTP}


## Title
additional title of region (used in cli output and outline view). It is possible to define title in region delimiter

<<< @../../examples/metaData/title.http{1 HTTP}

<<< @../../examples/metaData/title_shorthand.http{1 HTTP}
...
<<< @../../examples/metaData/description.http{1 HTTP}

::: tip
first comment of a region is automatically used as description
:::

<<< @../../examples/metaData/description_comment.http{1-3 HTTP}
...
<<< @../../examples/metaData/ref.http{1,5,10 HTTP}
...
<<< @../../examples/metaData/import.http{1 HTTP}
...
<<< @../../examples/metaData/loopForOf.http{5 HTTP}

### for ...

Variable `$index` is injected in Variables for every iteration

<<< @../../examples/metaData/loopFor.http{1 HTTP}

### while ...

Variable `$index` is injected in Variables for every iteration

<<< @../../examples/metaData/loopWhile.http{7 HTTP}
...
<<< @../../examples/metaData/loopAccessRequests.http{6 HTTP}
...
<<< @../../examples/metaData/save.http{1 HTTP}
...
<<< @../../examples/metaData/openWith.http{1 HTTP}
...
<<< @../../examples/metaData/extension.http{2 HTTP}
...
<<< @../../examples/metaData/noLog.http{1 HTTP}

## No Response View

Prevent open response in editor of VSCode

<<< @../../examples/metaData/noResponseView.http{1 HTTP}

## No Streaming Log

Prevent logging of intermediate responses while streaming data in output console

<<< @../../examples/metaData/noStreamingLog.http{3 HTTP}
...
<<< @../../examples/metaData/noClientCert.http{1 HTTP}

## No Redirect

Prevent following Redirects

<<< @../../examples/metaData/noRedirect.http{1 HTTP}
...
<<< @../../examples/metaData/proxy.http{1 HTTP}
...
<<< @../../examples/metaData/noproxy.http{1 HTTP}
...
<<< @../../examples/metaData/debug.http{1 HTTP}

## Verbose

enable trace log level

<<< @../../examples/metaData/verbose.http{1 HTTP}

## keepStreaming

keep streaming of MQTT, Server-Sent-Events or WebSocket until the session is ended manually. 

<<< @../../examples/metaData/keepStreaming.http{1 HTTP}
